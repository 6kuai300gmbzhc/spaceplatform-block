...
for _,planet_name in ipairs({"fulgora","gleba","vulcanus"}) do
    ...
    local main_connection_map = map_definitions(main_connection.asteroid_spawn_definitions)
    ...
    for _, asteroid_type in pairs(asteroid_types) do
        ...
        for _, connection in pairs(data.raw["space-connection"]) do
            ...
            local map = map_definitions(connection.asteroid_spawn_definitions)
            ...
            elseif connection.from == planet_name then
                local chunk = map[chunk_name].spawn_points
                local medium = map[medium_name].spawn_points
            ...
            elseif connection.to == "nauvis" then
                local chunk = map[chunk_name].spawn_points
                local medium = map[medium_name].spawn_points
            ...
            elseif connection.to == planet_name then
                local chunk = map[chunk_name].spawn_points
                local medium = map[medium_name].spawn_points
